...
class PlaceBuilder
{
	...
	): Entity
	{
		...
		(
			cellPosOfStart.clone().add
			(
				Coords.fromXYZ(0, -1, 0).multiplyScalar(.125)
			).multiply
			(
				maze.cellSizeInPixels
			),
	...
	static materialsCreate(): Material[]
	{
		...
		[
			...
			(
				"Chest",
				imageBuilder.squareOfColorWithInsetBorderOfColor
				(
					colorGray, colorRed
				)
			),
			...
			(
				"Wall",
				imageBuilder.wallMasonryWithColorsForBlocksAndMortar
				(
	...
	{
		var zonesForConnectorsToNeighbors: Zone2[] = [];
		var zonesAdjacentNames: string[] = [];
		...
		for (var n = 0; n < numberOfNeighbors; n++)
		{
			...
			{
				var conectorNameAndZone = this.zonesBuild_Cell_Neighbors_N
				(
					maze,
					cellPos,
					cellCurrent,
				...
				var connectorName = conectorNameAndZone[0] as string;
				var connectorZone = conectorNameAndZone[1] as Zone2;
	...
	static zonesBuild_Cell_Neighbors_N
	(
		maze: Maze,
		cellPos: Coords,
		cellCurrent: any,
	...
	{
		var zoneForConnector: Zone2 = null;
		...
		if (neighborIsOdd)
		{
			...
			zoneForConnector = Zone2.fromNamePosNeighborNamesAndEntities
			(
				zoneConnectorName,
