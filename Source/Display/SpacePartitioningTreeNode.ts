...
{
	faces: FaceTextured[];
	children: SpacePartitioningTreeNode[];

	constructor(faces: FaceTextured[])
	{
		...
		else
		{
			...
			for (var f = 1; f < faces.length; f++)
			{
				...
				else
				{
					...
					var facesDividedFrontAndBack =
						meshBuilder.splitFaceByPlaneFrontAndBack
						(
							faceOther,
							planeToDivideOn
						);
	...
	(
		cameraPos: Coords, facesToAddTo: FaceTextured[]
	): void
	{
		...
		else
		{
			var distanceOfCameraAbovePlane =
				this.faces[0].geometry.plane().distanceToPointAlongNormal(cameraPos);
